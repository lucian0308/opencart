...
class Affiliate {
 ...
 . " WHERE affiliate_id = '" . (int)$this->session->data['affiliate_id'] . "' AND status = '1'");
			...
			if ($affiliate_query->num_rows) {
				...
				$this->db->query("UPDATE " . DB_PREFIX . "affiliate SET ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "' " 
 . " WHERE affiliate_id = '" . (int)$this->session->data['affiliate_id'] . "'");
			} else {
				$this->logout();
			}
		}
