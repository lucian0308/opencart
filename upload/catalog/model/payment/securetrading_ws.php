...
class ModelPaymentSecureTradingWs extends Model {
 ...
 . " WHERE geo_zone_id = '" . (int)$this->config->get('securetrading_ws_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
 ...
 . " WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	public function removeMd($md) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "securetrading_ws_order " 
 . " WHERE md = '" . $this->db->escape($md) . "'");
	}

	public function updateReference($order_id, $transaction_reference) {
		$this->db->query("UPDATE " . DB_PREFIX . "securetrading_ws_order SET transaction_reference = '" . $this->db->escape($transaction_reference) . "' " 
 . " WHERE order_id = " . (int)$order_id);
	...
	public function getOrderId($md) {
		$row = $this->db->query("SELECT order_id FROM " . DB_PREFIX . "securetrading_ws_order " 
 . " WHERE md = '" . $this->db->escape($md) . "' LIMIT 1")->row;
 ...
 . " WHERE order_id = " . (int)$order_id);
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_ws_order` SET `settle_type`='" . $this->config->get('securetrading_ws_settle_status') . "', `modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $amount . "' " 
