...
class ModelPaymentSecureTradingPp extends Model {
 ...
 . " WHERE geo_zone_id = '" . (int)$this->config->get('securetrading_pp_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
 ...
 . " WHERE order_id = " . (int)$order_id);
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_pp_order` SET `settle_type`='" . $this->config->get('securetrading_pp_settle_status') . "', `modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $amount . "' " 
 ...
 . " WHERE order_id = " . (int)$order_id);
	...
	public function getCountry($iso_code_2) {
		return $this->db->query("SELECT * FROM " . DB_PREFIX . "country " 
 . " WHERE LOWER(iso_code_2) = '" . $this->db->escape($iso_code_2) . "'")->row;
	}
