...
class ModelPaymentPPPayflowIframe extends Model {
 ...
 . " WHERE geo_zone_id = '" . (int)$this->config->get('pp_payflow_iframe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
	...
	public function getOrderId($secure_token_id) {
		$result = $this->db->query("SELECT `order_id` FROM `" . DB_PREFIX . "paypal_payflow_iframe_order` " 
 . " WHERE `secure_token_id` = '" . $this->db->escape($secure_token_id) . "'")->row;
	...
	public function updateOrder($data) {
		$this->db->query(" UPDATE `" . DB_PREFIX . "paypal_payflow_iframe_order` SET `transaction_reference` = '" . $this->db->escape($data['transaction_reference']) . "', `transaction_type` = '" . $this->db->escape($data['transaction_type']) . "', `complete` = " . (int)$data['complete'] . " " 
