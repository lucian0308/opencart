...
class ModelAccountRecurring extends Model {
	...
	public function getProfile($id) {
		$result = $this->db->query("SELECT `or`.*,`o`.`payment_method`,`o`.`payment_code`,`o`.`currency_code` FROM `" . DB_PREFIX . "order_recurring` `or` " 
 . " LEFT JOIN `" . DB_PREFIX . "order` `o` ON `or`.`order_id` = `o`.`order_id` " 
 . " WHERE `or`.`order_recurring_id` = '" . (int)$id . "' AND `o`.`customer_id` = '" . (int)$this->customer->getId() . "' LIMIT 1");
	...
	public function getProfileByRef($ref) {
		$recurring = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_recurring` " 
 . " WHERE `reference` = '" . $this->db->escape($ref) . "' LIMIT 1");
	...
	public function getProfileTransactions($id) {
		...
		$results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_recurring_transaction` " 
 . " WHERE `order_recurring_id` = '" . (int)$id . "'");
	...
	public function getAllProfiles($start = 0, $limit = 20) {
		...
		$result = $this->db->query("SELECT `or`.*,`o`.`payment_method`,`o`.`currency_id`,`o`.`currency_value` FROM `" . DB_PREFIX . "order_recurring` `or` " 
 . " LEFT JOIN `" . DB_PREFIX . "order` `o` ON `or`.`order_id` = `o`.`order_id` " 
 ...
 . " ORDER BY `o`.`order_id` DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function getTotalRecurring() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order_recurring` `or` " 
 . " LEFT JOIN `" . DB_PREFIX . "order` `o` ON `or`.`order_id` = `o`.`order_id` " 
 . " WHERE `o`.`customer_id` = '" . (int)$this->customer->getId() . "'");
