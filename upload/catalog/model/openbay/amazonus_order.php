...
class ModelOpenbayAmazonusOrder extends Model {
 ...
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `subtract` = '1'");
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "product_option_variant` SET `stock` = GREATEST(`stock` - '" . (int)$delta . "', 0) " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `sku` = '" . $this->db->escape($var) . "' AND `subtract` = '1'");
		}
	}
