...
class ModelOpenbayAmazonusOrder extends Model {
  ...
  <MerchantOrderId>$order_id</MerchantOrderId>
    ...
    public function getProductId($sku) {
        $row = $this->db->query("SELECT `product_id` FROM `" . DB_PREFIX . "amazonus_product_link` "
                        . " WHERE `amazonus_sku` = '" . $this->db->escape($sku) . "'")->row;
    ...
    public function getProductVar($sku) {
        $row = $this->db->query("SELECT `var` FROM `" . DB_PREFIX . "amazonus_product_link` "
                        . " WHERE `amazonus_sku` = '" . $this->db->escape($sku) . "'")->row;
    ...
    public function decreaseProductQuantity($product_id, $delta, $var = '') {
        ...
        if ($var == '') {
            $this->db->query("UPDATE `" . DB_PREFIX . "product` SET `quantity` = GREATEST(`quantity` - '" . (int) $delta . "', 0) "
                    . " WHERE `product_id` = '" . (int) $product_id . "' AND `subtract` = '1'");
        } else {
            $this->db->query("UPDATE `" . DB_PREFIX . "product_option_relation` SET `stock` = GREATEST(`stock` - '" . (int) $delta . "', 0) "
                    . " WHERE `product_id` = '" . (int) $product_id . "' AND `var` = '" . $this->db->escape($var) . "' AND `subtract` = '1'");
        }
    }
    ...
    public function getCountryName($country_code) {
        $row = $this->db->query("SELECT `name` FROM `" . DB_PREFIX . "country` "
                        . " WHERE LOWER(`iso_code_2`) = '" . $this->db->escape(strtolower($country_code)) . "'")->row;
    ...
    public function getCountryId($country_code) {
        $row = $this->db->query("SELECT `country_id` FROM `" . DB_PREFIX . "country` "
                        . " WHERE LOWER(`iso_code_2`) = '" . $this->db->escape(strtolower($country_code)) . "'")->row;
    ...
    public function getZoneId($zone_name) {
        $row = $this->db->query("SELECT `zone_id` FROM `" . DB_PREFIX . "zone` "
                        . " WHERE LOWER(`name`) = '" . $this->db->escape(strtolower($zone_name)) . "'")->row;
    ...
    public function addAmazonusOrderProducts($order_id, $data) {
        foreach ($data as $sku => $order_item_id) {
            ...
            $row = $this->db->query("SELECT `order_product_id` FROM `" . DB_PREFIX . "order_product` "
                            . " WHERE `model` = '" . $this->db->escape($sku) . "' AND `order_id` = " . (int) $order_id . "LIMIT 1")->row;
    ...
    public function getOrderId($amazonus_order_id) {
        $row = $this->db->query("SELECT `order_id` FROM `" . DB_PREFIX . "amazonus_order` "
                        . " WHERE `amazonus_order_id` = '" . $this->db->escape($amazonus_order_id) . "' LIMIT 1")->row;
    ...
    public function getOrderStatus($order_id) {
        $row = $this->db->query("SELECT `order_status_id` FROM `" . DB_PREFIX . "order` "
    ...
    public function getProductOptionsByVar($product_var) {
        ...
        foreach ($option_value_ids as $option_value_id) {
            $option_details_row = $this->db->query("SELECT pov.product_option_id, pov.product_option_value_id, od.name, ovd.name as value, opt.type FROM `" . DB_PREFIX . "product_option_value` as pov, `" . DB_PREFIX . "product_option` as po, `" . DB_PREFIX . "option` as opt, `" . DB_PREFIX . "option_value_description` as ovd, `" . DB_PREFIX . "option_description` as od "
                            . " WHERE pov.product_option_value_id = '" . (int) $option_value_id . "' AND po.product_option_id = pov.product_option_id AND opt.option_id = pov.option_id AND ovd.option_value_id = pov.option_value_id AND ovd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND od.option_id = pov.option_id AND od.language_id = '" . (int) $this->config->get('config_language_id') . "' ")->row;
