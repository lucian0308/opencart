...
class ModelOpenbayAmazonOrder extends Model {
  ...
  <MerchantOrderId>$order_id</MerchantOrderId>
    ...
    public function getProductId($sku) {
        $row = $this->db->query("SELECT `product_id` FROM `" . DB_PREFIX . "amazon_product_link` "
                        . " WHERE `amazon_sku` = '" . $this->db->escape($sku) . "'")->row;
    ...
    public function getProductVar($sku) {
        $row = $this->db->query("SELECT `var` FROM `" . DB_PREFIX . "amazon_product_link` "
                        . " WHERE `amazon_sku` = '" . $this->db->escape($sku) . "'")->row;
    ...
    public function getOrderId($amazon_order_id) {
        $row = $this->db->query("SELECT `order_id` FROM `" . DB_PREFIX . "amazon_order` "
                        . " WHERE `amazon_order_id` = '" . $this->db->escape($amazon_order_id) . "' LIMIT 1")->row;
