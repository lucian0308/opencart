...
class ModelFraudIp extends Model {
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "fraud_ip` ( `ip` varchar(40) NOT NULL, `date_added` datetime NOT NULL, PRIMARY KEY (`ip`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
		");
	}
    ...
    public function removeIp($ip) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "fraud_ip` " 
 . " WHERE `ip` = '" . $this->db->escape($ip) . "'");
	...
	public function getIps($start = 0, $limit = 10) {
        ...
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "fraud_ip` " 
 . " ORDER BY `ip` ASC LIMIT " . (int)$start . "," . (int)$limit);
