...
class ModelCatalogCategory extends Model {
 ...
 . " ORDER BY `level` ASC");
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() " 
 . " WHERE category_id = '" . (int)$category_id . "'");
 ...
 . " ORDER BY level ASC");
	...
	public function getCategory($category_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT GROUP_CONCAT(cd1.name " 
 . " ORDER BY level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') FROM " . DB_PREFIX . "category_path cp " 
 ...
 . " GROUP BY cp.category_id) AS path, (SELECT DISTINCT keyword FROM " . DB_PREFIX . "url_alias " 
 . " WHERE query = 'category_id=" . (int)$category_id . "') AS keyword FROM " . DB_PREFIX . "category c " 
 ...
 . " WHERE c.category_id = '" . (int)$category_id . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getCategories($data = array()) {
		$sql = "SELECT cp.category_id AS category_id, GROUP_CONCAT(cd1.name " 
 . " ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "category_path cp " 
 ...
 . " WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategoryStores($category_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_to_store " 
 . " WHERE category_id = '" . (int)$category_id . "'");
 ...
 . " WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getTotalCategoriesByLayoutId($layout_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category_to_layout " 
