...
class ModelCatalogRecurring extends Model {
 ...
 . " WHERE recurring_id = '" . (int)$recurring_id . "'");
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "recurring` SET `price` = '" . (float)$data['price'] . "', `frequency` = '" . $this->db->escape($data['frequency']) . "', `duration` = '" . (int)$data['duration'] . "', `cycle` = '" . (int)$data['cycle'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `status` = '" . (int)$data['status'] . "', `trial_price` = '" . (float)$data['trial_price'] . "', `trial_frequency` = '" . $this->db->escape($data['trial_frequency']) . "', `trial_duration` = '" . (int)$data['trial_duration'] . "', `trial_cycle` = '" . (int)$data['trial_cycle'] . "', `trial_status` = '" . (int)$data['trial_status'] . "' " 
 . " WHERE recurring_id = '" . (int)$recurring_id . "'");
	...
	public function deleteRecurring($recurring_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "recurring` " 
 . " WHERE recurring_id = " . (int)$recurring_id . "");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "recurring_description` " 
 . " WHERE recurring_id = " . (int)$recurring_id . "");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_recurring` " 
 . " WHERE recurring_id = " . (int)$recurring_id . "");
		$this->db->query("UPDATE `" . DB_PREFIX . "order_recurring` SET `recurring_id` = 0 " 
 . " WHERE `recurring_id` = " . (int)$recurring_id . "");
	...
	public function getRecurring($recurring_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "recurring` " 
 . " WHERE recurring_id = '" . (int)$recurring_id . "'");
 ...
 . " WHERE `recurring_id` = '" . (int)$recurring_id . "'");
	...
	public function getRecurrings($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "recurring` r " 
