...
class ModelCatalogFilter extends Model {
	...
	public function editFilter($filter_group_id, $data) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "filter_group` SET sort_order = '" . (int)$data['sort_order'] . "' " 
 ...
 . " WHERE filter_group_id = '" . (int)$filter_group_id . "'");
	...
	public function deleteFilter($filter_group_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_group` " 
 . " WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_group_description` " 
 . " WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "filter` " 
 ...
 . " WHERE fg.filter_group_id = '" . (int)$filter_group_id . "' AND fgd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
 ...
 . " WHERE f.filter_group_id = fgd.filter_group_id AND fgd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS `group` FROM " . DB_PREFIX . "filter f " 
 . " LEFT JOIN " . DB_PREFIX . "filter_description fd ON (f.filter_id = fd.filter_id) " 
 . " WHERE f.filter_id = '" . (int)$filter_id . "' AND fd.language_id = '" . (int)$this->config->get('config_language_id') . "'");

		return $query->row;
	}

	public function getFilters($data) {
		$sql = "SELECT *, (SELECT name FROM " . DB_PREFIX . "filter_group_description fgd " 
 . " WHERE f.filter_group_id = fgd.filter_group_id AND fgd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS `group` FROM " . DB_PREFIX . "filter f " 
 . " LEFT JOIN " . DB_PREFIX . "filter_description fd ON (f.filter_id = fd.filter_id) " 
 ...
 . " WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		...
		foreach ($filter_query->rows as $filter) {
			...
			$filter_description_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "filter_description " 
