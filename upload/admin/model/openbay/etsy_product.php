...
class ModelOpenbayEtsyProduct extends Model{
	public function getStatus($product_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "etsy_listing` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `status` = 1 LIMIT 1");
	...
	public function totalLinked() {
		$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM `" . DB_PREFIX . "etsy_listing` `el` " 
 ...
 . " WHERE `el`.`status` = '1' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function loadLinked($limit = 100, $page = 1) {
		...
		$sql = " SELECT `el`.`etsy_listing_id`, `el`.`etsy_item_id`, `el`.`status`, `p`.`product_id`, `p`.`sku`, `p`.`model`, `p`.`quantity`, `pd`.`name` FROM `" . DB_PREFIX . "etsy_listing` `el` " 
 . " LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`el`.`product_id` = `p`.`product_id`) " 
 . " LEFT JOIN `" . DB_PREFIX . "product_description` `pd` ON (`p`.`product_id` = `pd`.`product_id`) " 
 . " WHERE `el`.`status` = '1' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
