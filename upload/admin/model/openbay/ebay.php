...
{country}';
		...
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_category` ( `ebay_category_id` int(11) NOT NULL AUTO_INCREMENT, `CategoryID` int(11) NOT NULL, `CategoryParentID` int(11) NOT NULL, `CategoryLevel` smallint(6) NOT NULL, `CategoryName` char(100) NOT NULL, `BestOfferEnabled` tinyint(1) NOT NULL, `AutoPayEnabled` tinyint(1) NOT NULL, PRIMARY KEY (`ebay_category_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_category_history` ( `ebay_category_history_id` int(11) NOT NULL AUTO_INCREMENT, `CategoryID` int(11) NOT NULL, `breadcrumb` varchar(255) NOT NULL, `used` int(6) NOT NULL, PRIMARY KEY (`ebay_category_history_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_listing` ( `ebay_listing_id` int(11) NOT NULL AUTO_INCREMENT, `ebay_item_id` char(100) NOT NULL, `product_id` int(11) NOT NULL, `variant` int(11) NOT NULL, `status` SMALLINT(3) NOT NULL DEFAULT '1', PRIMARY KEY (`ebay_listing_id`), KEY `product_id` (`product_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");
		;
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_listing_pending` ( `ebay_listing_pending_id` int(11) NOT NULL AUTO_INCREMENT, `ebay_item_id` char(25) NOT NULL, `product_id` int(11) NOT NULL, `key` char(50) NOT NULL, `variant` int(11) NOT NULL, PRIMARY KEY (`ebay_listing_pending_id`), KEY `product_id` (`product_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_shipping` ( `ebay_shipping_id` int(11) NOT NULL AUTO_INCREMENT, `description` varchar(100) NOT NULL, `InternationalService` tinyint(4) NOT NULL, `ShippingService` varchar(100) NOT NULL, `ShippingServiceID` int(11) NOT NULL, `ServiceType` varchar(100) NOT NULL, `ValidForSellingFlow` tinyint(4) NOT NULL, `ShippingCategory` varchar(100) NOT NULL, `ShippingTimeMin` int(11) NOT NULL, `ShippingTimeMax` int(11) NOT NULL, PRIMARY KEY (`ebay_shipping_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");
		...
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_payment_method` ( `ebay_payment_method_id` int(11) NOT NULL AUTO_INCREMENT, `ebay_name` char(50) NOT NULL, `local_name` char(50) NOT NULL, PRIMARY KEY (`ebay_payment_method_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1 AUTO_INCREMENT=5;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_transaction` ( `ebay_transaction_id` int(11) NOT NULL AUTO_INCREMENT, `order_id` int(11) NOT NULL, `product_id` int(11) NOT NULL, `sku` varchar(100) NOT NULL, `txn_id` varchar(100) NOT NULL, `item_id` varchar(100) NOT NULL, `containing_order_id` varchar(100) NOT NULL, `order_line_id` varchar(100) NOT NULL, `qty` int(11) NOT NULL, `smp_id` int(11) NOT NULL, `created` DATETIME NOT NULL, `modified` DATETIME NOT NULL, PRIMARY KEY (`ebay_transaction_id`), KEY `product_id` (`product_id`), KEY `order_id` (`order_id`), KEY `smp_id` (`smp_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_order` ( `ebay_order_id` int(11) NOT NULL AUTO_INCREMENT, `parent_ebay_order_id` int(11) NOT NULL, `order_id` int(11) NOT NULL, `smp_id` int(11) NOT NULL, `tracking_no` varchar(100) NOT NULL, `carrier_id` varchar(100) NOT NULL, PRIMARY KEY (`ebay_order_id`), KEY `order_id` (`order_id`), KEY `smp_id` (`smp_id`), KEY `parent_ebay_order_id` (`parent_ebay_order_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_profile` ( `ebay_profile_id` int(11) NOT NULL AUTO_INCREMENT, `name` varchar(100) NOT NULL, `description` text NOT NULL, `type` int(11) NOT NULL, `default` TINYINT(1) NOT NULL, `data` text NOT NULL, PRIMARY KEY (`ebay_profile_id`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1;");
		...
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "ebay_stock_reserve` ( `id` int(11) NOT NULL AUTO_INCREMENT, `product_id` int(11) NOT NULL, `variant_id` varchar(100) NOT NULL, `item_id` varchar(100) NOT NULL, `reserve` int(11) NOT NULL, PRIMARY KEY (`id`), KEY `product_id` (`product_id`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8;");
	...
	public function totalLinked() {
		$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM `" . DB_PREFIX . "ebay_listing` `el` " 
 ...
 . " WHERE `el`.`status` = '1' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function loadLinked($limit = 100, $page = 1) {
		...
		$sql = " SELECT " . $has_option . " `el`.`ebay_item_id`, `p`.`product_id`, `p`.`sku`, `p`.`model`, `p`.`quantity`, `pd`.`name`, `esr`.`reserve` FROM `" . DB_PREFIX . "ebay_listing` `el` " 
 . " LEFT JOIN `" . DB_PREFIX . "product` `p` ON (`el`.`product_id` = `p`.`product_id`) " 
 . " LEFT JOIN `" . DB_PREFIX . "product_description` `pd` ON (`p`.`product_id` = `pd`.`product_id`) " 
 ...
 . " WHERE `el`.`status` = '1' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getSellerStoreCategories() {
		...
		if ($qry->num_rows) {
			$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_store_category` " 
 . " WHERE `parent_id` = '0' " 
 . " ORDER BY `CategoryName` ASC");
			...
			if ($qry->num_rows) {
				...
				foreach ($qry->rows as $row) {
					...
					$qry2 = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_store_category` " 
 . " WHERE `parent_id` = '" . $row['ebay_store_category_id'] . "' " 
 . " ORDER BY `CategoryName` ASC");

					if ($qry2->num_rows) {
						foreach ($qry2->rows as $row2) {
							$qry3 = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_store_category` " 
 . " WHERE `parent_id` = '" . $row2['ebay_store_category_id'] . "' " 
 . " ORDER BY `CategoryName` ASC");
	...
	public function getCategory($parent) {
		...
		if (empty($parent)) {
			$cat_qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_category` " 
 ...
 . " WHERE `CategoryParentID` = '" . $parent . "'");
	...
	public function getShippingService($international, $type) {
		$json = array();
		$result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_shipping` " 
 . " WHERE `InternationalService` = '" . (int)$international . "' AND `ValidForSellingFlow` = '1' AND `ServiceType` LIKE '%" . $this->db->escape($type) . "%'");
	...
	public function getShippingLocations() {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_shipping_location` " 
 . " WHERE `shipping_location` != 'None' AND `shipping_location` != 'Worldwide'");
	...
	public function getPopularCategories() {
		$res    = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_category_history` " 
 . " ORDER BY `used` DESC LIMIT 5");
	...
	private function getCategoryStructure($id) {
		$res = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_category` " 
 . " WHERE `CategoryID` = '" . $this->db->escape($id) . "' LIMIT 1");
		return $res->row;
	}
	...
	public function logCategoryUsed($category_id) {
		...
		if ($err == false) {
			$res = $this->db->query("SELECT `used` FROM `" . DB_PREFIX . "ebay_category_history` " 
 . " WHERE `CategoryID` = '" . $original_id . "' LIMIT 1");

			if ($res->num_rows) {
				$new = $res->row['used'] + 1;
				$this->db->query("UPDATE `" . DB_PREFIX . "ebay_category_history` SET `used` = '" . $new . "' " 
 . " WHERE `CategoryID` = '" . $original_id . "' LIMIT 1");
			} else {
 ...
 . " WHERE `product_id` = '" . $this->db->escape($id) . "' LIMIT 1");
	...
	public function getLiveListingArray() {
		$qry = $this->db->query("SELECT `product_id`, `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` " 
 . " WHERE `status` = 1");
 ...
 . " WHERE pa.product_id = '" . (int)$product_id . "' AND a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' " 
