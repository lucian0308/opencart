...
class ModelOpenbayAmazonus extends Model {
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_order` ( `order_id` int(11) NOT NULL , `amazonus_order_id` char(19) NOT NULL , `courier_id` varchar(255) NOT NULL , `courier_other` tinyint(1) NOT NULL, `tracking_no` varchar(255) NOT NULL , PRIMARY KEY (`order_id`, `amazonus_order_id`) ) DEFAULT COLLATE=utf8_general_ci;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_order_product` ( `order_product_id` int(11) NOT NULL , `amazonus_order_item_id` varchar(255) NOT NULL, PRIMARY KEY(`order_product_id`, `amazonus_order_item_id`) );");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_product_unshipped` ( `order_id` int(11) NOT NULL, `product_id` int(11) NOT NULL, `quantity` int(11) NOT NULL DEFAULT '0', PRIMARY KEY (`order_id`,`product_id`) ) DEFAULT COLLATE=utf8_general_ci;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_product` ( `version` int(11) NOT NULL DEFAULT 2, `product_id` int(11) NOT NULL , `category` varchar(255) NOT NULL , `sku` varchar(255) NOT NULL , `insertion_id` varchar(255) NOT NULL , `data` text NOT NULL , `status` enum('saved','uploaded','ok','error') NOT NULL , `price` decimal(15,4) NOT NULL COMMENT 'Price on Amazonus' , `var` char(100) NOT NULL DEFAULT '', `marketplaces` text NOT NULL , `messages` text NOT NULL, PRIMARY KEY (`product_id`, `var`) );");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_product_error` ( `error_id` int(11) NOT NULL AUTO_INCREMENT, `sku` varchar(255) NOT NULL , `insertion_id` varchar(255) NOT NULL , `error_code` int(11) NOT NULL , `message` text NOT NULL , PRIMARY KEY (`error_id`) );");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_product_link` ( `id` int(11) NOT NULL AUTO_INCREMENT, `amazonus_sku` varchar(255) NOT NULL, `var` char(100) NOT NULL DEFAULT '', `product_id` int(11) NOT NULL, PRIMARY KEY (`id`) ) DEFAULT COLLATE=utf8_general_ci;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazonus_product_search` ( `product_id` int(11) NOT NULL, `status` enum('searching','finished') NOT NULL, `matches` int(11) DEFAULT NULL, `data` text, PRIMARY KEY (`product_id`) ) DEFAULT COLLATE=utf8_general_ci;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS`" . DB_PREFIX . "amazonus_listing_report` ( `sku` varchar(255) NOT NULL, `quantity` int(10) unsigned NOT NULL, `asin` varchar(255) NOT NULL, `price` decimal(10,4) NOT NULL, PRIMARY KEY (`sku`) ) DEFAULT COLLATE=utf8_general_ci;
		");
 ...
 . " WHERE `code` = 'openbay_amazonus'");
  ...
  <MarketplaceIDs>';
	...
	public function saveProduct($product_id, $data_array) {
		...
		$this->db->query(" REPLACE INTO `" . DB_PREFIX . "amazonus_product` SET `product_id` = '" . (int)$product_id . "', `sku` = '" . $this->db->escape($sku) . "', `category` = '" . $this->db->escape($category) . "', `data` = '" . $this->db->escape($data_encoded) . "', `status` = 'saved', `insertion_id` = '', `price` = '" . $price . "', `var` = '" . $this->db->escape($var) . "', `marketplaces` = '" . $this->db->escape($marketplaces) . "'");
	}
 ...
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `var` = '" . $this->db->escape($var) . "'");
	...
	public function getSavedProducts() {
		return $this->db->query(" SELECT `ap`.`status`, `ap`.`product_id`, `ap`.`sku` as `amazonus_sku`, `pd`.`name` as `product_name`, `p`.`model` as `product_model`, `p`.`sku` as `product_sku`, `ap`.`var` as `var` FROM `" . DB_PREFIX . "amazonus_product` as `ap` " 
 ...
 . " WHERE `ap`.`status` = 'saved' AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'")->rows;
	...
	public function getSavedProductsData() {
		return $this->db->query(" SELECT * FROM `" . DB_PREFIX . "amazonus_product` " 
 ...
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `var` = '" . $this->db->escape($var) . "' AND `version` = 2")->row;
	...
	public function getProductCategory($product_id, $var = '') {
		$row = $this->db->query(" SELECT `category` FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `var` = '" . $this->db->escape($var) . "' AND `version` = 2")->row;
	...
	public function getProductStatus($product_id) {
		...
		$rows_uploaded = $this->db->query(" SELECT COUNT(*) count FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND status = 'uploaded'")->row;
		$rows_uploaded = $rows_uploaded['count'];

		$rows_ok = $this->db->query(" SELECT COUNT(*) count FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND status = 'ok'")->row;
		$rows_ok = $rows_ok['count'];

		$rows_error = $this->db->query(" SELECT COUNT(*) count FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND status = 'error'")->row;
		$rows_error = $rows_error['count'];

		$rows_saved = $this->db->query(" SELECT COUNT(*) count FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND status = 'saved'")->row;
		...
		$links = $this->db->query(" SELECT COUNT(*) as count FROM `" . DB_PREFIX . "amazonus_product_link` " 
 . " WHERE `product_id` = '" . (int)$product_id . "'")->row;
		...
		if ($rows_uploaded == 0 && $rows_error > 0 && $rows_ok == 0) {
			$quick = $this->db->query("SELECT * FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = " . (int)$product_id . " AND `version` = 3")->row;
	...
	public function getProductErrors($product_id, $version = 2) {
		if ($version == 3) {
			$message_row = $this->db->query(" SELECT `messages` FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `version` = 3")->row;
		...
		$insertion_rows = $this->db->query("SELECT `sku`, `insertion_id` FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `version` = 2")->rows;

		if (!empty($insertion_rows)) {
			foreach($insertion_rows as $insertion_row) {
				$error_rows = $this->db->query("SELECT * FROM `" . DB_PREFIX . "amazonus_product_error` " 
 . " WHERE `sku` = '" . $this->db->escape($insertion_row['sku']) . "' AND `insertion_id` = '" . $this->db->escape($insertion_row['insertion_id']) . "'")->rows;
	...
	public function getProductsWithErrors() {
		return $this->db->query(" SELECT `product_id`, `sku` FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `status` = 'error' AND `version` = 2")->rows;
	...
	public function linkProduct($amazonus_sku, $product_id, $var = '') {
		$count = $this->db->query("SELECT COUNT(*) as 'count' FROM `" . DB_PREFIX . "amazonus_product_link` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `amazonus_sku` = '" . $this->db->escape($amazonus_sku) . "' AND `var` = '" . $this->db->escape($var) . "' LIMIT 1")->row;
		if ($count['count'] == 0) {
	...
	public function removeAdvancedErrors($product_id) {
		$product_rows = $this->db->query(" SELECT `insertion_id` FROM `" . DB_PREFIX . "amazonus_product` " 
 . " WHERE `product_id` = '" . (int)$product_id . "' AND `version` = 2")->rows;
 ...
 . " LEFT JOIN `" . DB_PREFIX . "amazonus_product_link` as `apl` ON `apl`.`product_id` = `p`.`product_id` " 
 . " WHERE `apl`.`amazonus_sku` IS NULL AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'")->rows;
		}
	...
	private function productLinkExists($product_id, $var) {
		$link = $this->db->query("SELECT * FROM `" . DB_PREFIX . "amazonus_product_link` " 
 ...
 . " WHERE `s`.`key` = 'openbay_amazonus_order_status_shipped' OR `s`.`key` = 'openbay_amazonus_order_status_canceled' LIMIT 1")->row;
	...
	public function updateAmazonusOrderTracking($order_id, $courier_id, $courier_from_list, $tracking_no) {
		$this->db->query(" UPDATE `" . DB_PREFIX . "amazonus_order` SET `courier_id` = '" . $courier_id . "', `courier_other` = " . (int)!$courier_from_list . ", `tracking_no` = '" . $tracking_no . "' " 
 . " WHERE `order_id` = " . (int)$order_id . "");
	}

	public function getAmazonusOrderId($order_id) {
		$row = $this->db->query(" SELECT `amazonus_order_id` FROM `" . DB_PREFIX . "amazonus_order` " 
 . " WHERE `order_id` = " . (int)$order_id . " LIMIT 1")->row;
	...
	public function getAmazonusOrderedProducts($order_id) {
		return $this->db->query(" SELECT `aop`.`amazonus_order_item_id`, `op`.`quantity` FROM `" . DB_PREFIX . "amazonus_order_product` `aop` JOIN `" . DB_PREFIX . "order_product` `op` ON `op`.`order_product_id` = `aop`.`order_product_id` AND `op`.`order_id` = " . (int)$order_id)->rows;
	}
 ...
 . " WHERE apl.product_id IS NULL AND ap.product_id IS NULL ";
	...
	public function getProductSearch($data = array()) {
		$sql = " SELECT p.product_id, aps.status, aps.data, aps.matches FROM " . DB_PREFIX . "product p " 
 . " LEFT JOIN " . DB_PREFIX . "amazonus_product_search aps ON p.product_id = aps.product_id " 
 . " LEFT JOIN " . DB_PREFIX . "amazonus_product_link apl ON p.product_id = apl.product_id " 
 . " LEFT JOIN " . DB_PREFIX . "amazonus_product ap ON p.product_id = ap.product_id " 
 . " WHERE apl.product_id IS NULL AND ap.product_id IS NULL ";
	...
	public function updateAmazonSkusQuantities($skus) {
		...
		if ($this->openbay->addonLoad('openstock')) {
			$rows = $this->db->query(" SELECT apl.amazonus_sku, if (por.product_id IS NULL, p.quantity, por.stock) AS 'quantity' FROM " . DB_PREFIX . "amazonus_product_link apl JOIN " . DB_PREFIX . "product p ON apl.product_id = p.product_id " 
 . " LEFT JOIN " . DB_PREFIX . "product_option_variant por ON apl.product_id = por.product_id AND apl.var = por.sku " 
 . " WHERE apl.amazonus_sku IN (" . implode(',', $sku_array) . ") ")->rows;
		} else {
			$rows = $this->db->query(" SELECT apl.amazonus_sku, p.quantity FROM " . DB_PREFIX . "amazonus_product_link apl JOIN " . DB_PREFIX . "product p ON apl.product_id = p.product_id " 
 . " WHERE apl.amazonus_sku IN (" . implode(',', $sku_array) . ") ")->rows;
		}
	...
	public function getTotalUnlinkedItemsFromReport() {
		if ($this->openbay->addonLoad('openstock')) {
			$result = $this->db->query(" SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', oc_sku.product_id, pd.name, oc_sku.sku, oc_sku.var, oc_sku.quantity, oc_sku.pov_id FROM " . DB_PREFIX . "amazonus_listing_report alr " 
 ...
 . " WHERE apl.product_id IS NULL ");
			...
			$result = $this->db->query(" SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', oc_sku.product_id, pd.name, oc_sku.sku, oc_sku.var, oc_sku.quantity, '' AS combination FROM " . DB_PREFIX . "amazonus_listing_report alr " 
 ...
 . " ORDER BY alr.sku ");
	...
	public function getUnlinkedItemsFromReport($limit = 100, $page = 1) {
		...
		if ($this->openbay->addonLoad('openstock')) {
			$this->load->model('module/openstock');
			$rows = $this->db->query(" SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', oc_sku.product_id, pd.name, oc_sku.sku, oc_sku.var, oc_sku.quantity, oc_sku.pov_id FROM " . DB_PREFIX . "amazonus_listing_report alr " 
 . " LEFT JOIN ( SELECT p.product_id, if (por.product_id IS NULL, p.sku, por.sku) AS 'sku', if (por.product_id IS NULL, NULL, por.sku) AS 'var', if (por.product_id IS NULL, p.quantity, por.stock) AS 'quantity', por.product_option_variant_id AS pov_id FROM " . DB_PREFIX . "product p " 
 . " LEFT JOIN " . DB_PREFIX . "product_option_variant por USING(product_id) ) AS oc_sku ON alr.sku = oc_sku.sku " 
 . " LEFT JOIN " . DB_PREFIX . "amazonus_product_link apl ON (oc_sku.var IS NULL AND oc_sku.product_id = apl.product_id) OR (oc_sku.var IS NOT NULL AND oc_sku.product_id = apl.product_id AND oc_sku.var = apl.var) " 
 . " LEFT JOIN " . DB_PREFIX . "product_description pd ON oc_sku.product_id = pd.product_id AND pd.language_id = " . (int)$this->config->get('config_language_id') . " " 
 ...
 . " ORDER BY alr.sku LIMIT " . (int)$start . "," . (int)$limit)->rows;
			...
			$rows = $this->db->query(" SELECT alr.sku AS 'amazon_sku', alr.quantity AS 'amazon_quantity', alr.asin, alr.price AS 'amazon_price', oc_sku.product_id, pd.name, oc_sku.sku, oc_sku.var, oc_sku.quantity, '' AS combination FROM " . DB_PREFIX . "amazonus_listing_report alr " 
 . " LEFT JOIN ( SELECT p.product_id, p.sku, NULL AS 'var', p.quantity FROM " . DB_PREFIX . "product p ) AS oc_sku ON alr.sku = oc_sku.sku " 
 . " LEFT JOIN " . DB_PREFIX . "amazonus_product_link apl ON (oc_sku.var IS NULL AND oc_sku.product_id = apl.product_id) OR (oc_sku.var IS NOT NULL AND oc_sku.product_id = apl.product_id AND oc_sku.var = apl.var) " 
 . " LEFT JOIN " . DB_PREFIX . "product_description pd ON oc_sku.product_id = pd.product_id AND pd.language_id = " . (int)$this->config->get('config_language_id') . " " 
 . " WHERE apl.product_id IS NULL " 
