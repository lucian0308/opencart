...
class ModelPaymentPPPayflowIFrame extends Model {
	...
	public function getOrder($order_id) {
		$result = $this->db->query("SELECT * FROM " . DB_PREFIX . "paypal_payflow_iframe_order " 
 . " WHERE order_id = " . (int)$order_id);
	...
	public function updateOrderStatus($order_id, $status) {
		$this->db->query(" UPDATE " . DB_PREFIX . "paypal_payflow_iframe_order SET `complete` = " . (int)$status . " " 
 . " WHERE order_id = '" . (int)$order_id . "' ");
	}
	...
	public function getTransactions($order_id) {
		return $this->db->query(" SELECT * FROM " . DB_PREFIX . "paypal_payflow_iframe_order_transaction " 
 . " WHERE order_id = " . (int)$order_id . " " 
 . " ORDER BY `time` ASC")->rows;
	}

	public function getTransaction($transaction_reference) {
		$result = $this->db->query(" SELECT * FROM " . DB_PREFIX . "paypal_payflow_iframe_order_transaction " 
