...
class ModelPaymentWorldpay extends Model {
	...
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "worldpay_order` ( `worldpay_order_id` INT(11) NOT NULL AUTO_INCREMENT, `order_id` INT(11) NOT NULL, `order_code` VARCHAR(50), `date_added` DATETIME NOT NULL, `date_modified` DATETIME NOT NULL, `refund_status` INT(1) DEFAULT NULL, `currency_code` CHAR(3) NOT NULL, `total` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`worldpay_order_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
	...
	public function updateRefundStatus($worldpay_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "worldpay_order` SET `refund_status` = '" . (int)$status . "' " 
 ...
 . " WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	private function getTransactions($worldpay_order_id, $currency_code) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "worldpay_order_transaction` " 
 . " WHERE `worldpay_order_id` = '" . (int)$worldpay_order_id . "'");
	...
	public function getTotalReleased($worldpay_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "worldpay_order_transaction` " 
 . " WHERE `worldpay_order_id` = '" . (int)$worldpay_order_id . "' AND (`type` = 'payment' OR `type` = 'refund')");
	...
	public function getTotalRefunded($worldpay_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "worldpay_order_transaction` " 
 . " WHERE `worldpay_order_id` = '" . (int)$worldpay_order_id . "' AND 'refund'");
