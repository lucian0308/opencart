...
class ModelPaymentSecureTradingPp extends Model {
	...
	public function updateVoidStatus($securetrading_pp_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_pp_order` SET `void_status` = '" . (int)$status . "' " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "'");
	}
	...
	public function updateReleaseStatus($securetrading_pp_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_pp_order` SET `release_status` = '" . (int)$status . "' " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "'");
	}

	public function updateForRebate($securetrading_pp_order_id, $order_ref) {
		$this->db->query("UPDATE `" . DB_PREFIX . "securetrading_pp_order` SET `order_ref_previous` = '_multisettle_" . $this->db->escape($order_ref) . "' " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "' LIMIT 1");
	}
	...
	public function getOrder($order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "securetrading_pp_order` " 
 . " WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	private function getTransactions($securetrading_pp_order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "securetrading_pp_order_transaction` " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "'");
	...
	public function getTotalReleased($securetrading_pp_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "securetrading_pp_order_transaction` " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "' AND (`type` = 'payment' OR `type` = 'rebate')");
	...
	public function getTotalRebated($securetrading_pp_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "securetrading_pp_order_transaction` " 
 . " WHERE `securetrading_pp_order_id` = '" . (int)$securetrading_pp_order_id . "' AND 'rebate'");

		return (double)$query->row['total'];
	}

	public function increaseRefundedAmount($order_id, $amount) {
		$this->db->query("UPDATE " . DB_PREFIX . "securetrading_pp_order SET refunded = refunded + " . (double)$amount . " " 
