...
class ModelPaymentG2aPay extends Model {
	...
	public function getOrder($order_id) {
		...
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "g2apay_order` " 
 . " WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	public function getTotalReleased($g2apay_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "g2apay_order_transaction` " 
 . " WHERE `g2apay_order_id` = '" . (int)$g2apay_order_id . "' AND (`type` = 'payment' OR `type` = 'refund')");
	...
	public function updateRefundStatus($g2apay_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "g2apay_order` SET `refund_status` = '" . (int)$status . "' " 
 . " WHERE `g2apay_order_id` = '" . (int)$g2apay_order_id . "'");
	}

	private function getTransactions($g2apay_order_id, $currency_code) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "g2apay_order_transaction` " 
 . " WHERE `g2apay_order_id` = '" . (int)$g2apay_order_id . "'");
	...
	public function getTotalRefunded($g2apay_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "g2apay_order_transaction` " 
 . " WHERE `g2apay_order_id` = '" . (int)$g2apay_order_id . "' AND 'refund'");
