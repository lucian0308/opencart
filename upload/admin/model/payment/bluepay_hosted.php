...
class ModelPaymentBluePayHosted extends Model {
    ...
    public function install() {
        $this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "bluepay_hosted_order` ( `bluepay_hosted_order_id` INT(11) NOT NULL AUTO_INCREMENT, `order_id` INT(11) NOT NULL, `transaction_id` VARCHAR(50), `date_added` DATETIME NOT NULL, `date_modified` DATETIME NOT NULL, `release_status` INT(1) DEFAULT 0, `void_status` INT(1) DEFAULT 0, `rebate_status` INT(1) DEFAULT 0, `currency_code` CHAR(3) NOT NULL, `total` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`bluepay_hosted_order_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");

        $this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "bluepay_hosted_order_transaction` ( `bluepay_hosted_order_transaction_id` INT(11) NOT NULL AUTO_INCREMENT, `bluepay_hosted_order_id` INT(11) NOT NULL, `date_added` DATETIME NOT NULL, `type` ENUM('auth', 'sale', 'rebate', 'void') DEFAULT NULL, `amount` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`bluepay_hosted_order_transaction_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
    ...
    public function updateVoidStatus($bluepay_hosted_order_id, $status) {
        ...
        $this->logger('$status:\r\n' . print_r($status, 1));
        $this->db->query("UPDATE `" . DB_PREFIX . "bluepay_hosted_order` SET `void_status` = '" . (int) $status . "' "
    ...
    public function updateReleaseStatus($bluepay_hosted_order_id, $status) {
        $this->db->query("UPDATE `" . DB_PREFIX . "bluepay_hosted_order` SET `release_status` = '" . (int) $status . "' "
                . " WHERE `bluepay_hosted_order_id` = '" . (int) $bluepay_hosted_order_id . "'");
    }
    ...
    public function updateRebateStatus($bluepay_hosted_order_id, $status) {
        $this->db->query("UPDATE `" . DB_PREFIX . "bluepay_hosted_order` SET `rebate_status` = '" . (int) $status . "' "
                . " WHERE `bluepay_hosted_order_id` = '" . (int) $bluepay_hosted_order_id . "'");
    }

    public function getOrder($order_id) {

        $qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "bluepay_hosted_order` "
    ...
    private function getTransactions($bluepay_hosted_order_id) {
        $qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "bluepay_hosted_order_transaction` "
                . " WHERE `bluepay_hosted_order_id` = '" . (int) $bluepay_hosted_order_id . "'");
    ...
    public function getTotalReleased($bluepay_hosted_order_id) {
        $query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "bluepay_hosted_order_transaction` "
                . " WHERE `bluepay_hosted_order_id` = '" . (int) $bluepay_hosted_order_id . "' AND (`type` = 'sale' OR `type` = 'rebate' OR `type` = 'auth')");
    ...
    public function getTotalRebated($bluepay_hosted_order_id) {
        $query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "bluepay_hosted_order_transaction` "
                . " WHERE `bluepay_hosted_order_id` = '" . (int) $bluepay_hosted_order_id . "' AND 'rebate'");
