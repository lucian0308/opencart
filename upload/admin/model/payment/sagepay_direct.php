...
class ModelPaymentSagepayDirect extends Model {
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sagepay_direct_order` ( `sagepay_direct_order_id` INT(11) NOT NULL AUTO_INCREMENT, `order_id` INT(11) NOT NULL, `VPSTxId` VARCHAR(50), `VendorTxCode` VARCHAR(50) NOT NULL, `SecurityKey` CHAR(50) NOT NULL, `TxAuthNo` INT(50), `date_added` DATETIME NOT NULL, `date_modified` DATETIME NOT NULL, `release_status` INT(1) DEFAULT NULL, `void_status` INT(1) DEFAULT NULL, `settle_type` INT(1) DEFAULT NULL, `rebate_status` INT(1) DEFAULT NULL, `currency_code` CHAR(3) NOT NULL, `total` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`sagepay_direct_order_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
		...
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "sagepay_direct_card` ( `card_id` INT(11) NOT NULL AUTO_INCREMENT, `customer_id` INT(11) NOT NULL, `order_id` INT(11) NOT NULL, `token` VARCHAR(50) NOT NULL, `digits` VARCHAR(4) NOT NULL, `expiry` VARCHAR(5) NOT NULL, `type` VARCHAR(50) NOT NULL, PRIMARY KEY (`card_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
	}
	...
	public function updateVoidStatus($sagepay_direct_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "sagepay_direct_order` SET `void_status` = '" . (int)$status . "' " 
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "'");
	...
	public function updateReleaseStatus($sagepay_direct_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "sagepay_direct_order` SET `release_status` = '" . (int)$status . "' " 
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "'");
	}
	...
	public function updateRebateStatus($sagepay_direct_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "sagepay_direct_order` SET `rebate_status` = '" . (int)$status . "' " 
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "'");
	}
 ...
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "'");
	...
	public function getTotalReleased($sagepay_direct_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "sagepay_direct_order_transaction` " 
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "' AND (`type` = 'payment' OR `type` = 'rebate')");
	...
	public function getTotalRebated($sagepay_direct_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "sagepay_direct_order_transaction` " 
 . " WHERE `sagepay_direct_order_id` = '" . (int)$sagepay_direct_order_id . "' AND 'rebate'");
