...
class ModelPaymentPPProIframe extends Model {
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "paypal_iframe_order` ( `paypal_iframe_order_id` int(11) NOT NULL AUTO_INCREMENT, `order_id` int(11) NOT NULL, `date_added` DATETIME NOT NULL, `date_modified` DATETIME NOT NULL, `capture_status` ENUM('Complete','NotComplete') DEFAULT NULL, `currency_code` CHAR(3) NOT NULL, `authorization_id` VARCHAR(30) NOT NULL, `total` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`paypal_iframe_order_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");

		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "paypal_iframe_order_transaction` ( `paypal_iframe_order_transaction_id` int(11) NOT NULL AUTO_INCREMENT, `paypal_iframe_order_id` int(11) NOT NULL, `transaction_id` CHAR(20) NOT NULL, `parent_transaction_id` CHAR(20) NOT NULL, `date_added` DATETIME NOT NULL, `note` VARCHAR(255) NOT NULL, `msgsubid` CHAR(38) NOT NULL, `receipt_id` CHAR(20) NOT NULL, `payment_type` ENUM('none','echeck','instant', 'refund', 'void') DEFAULT NULL, `payment_status` CHAR(20) NOT NULL, `pending_reason` CHAR(50) NOT NULL, `transaction_entity` CHAR(50) NOT NULL, `amount` DECIMAL( 10, 2 ) NOT NULL, `debug_data` TEXT NOT NULL, `call_data` TEXT NOT NULL, PRIMARY KEY (`paypal_iframe_order_transaction_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
	}
	...
	private function getTransactions($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT `ot`.*, ( SELECT count(`ot2`.`paypal_iframe_order_id`) FROM `" . DB_PREFIX . "paypal_iframe_order_transaction` `ot2` " 
 . " WHERE `ot2`.`parent_transaction_id` = `ot`.`transaction_id` ) AS `children` FROM `" . DB_PREFIX . "paypal_iframe_order_transaction` `ot` " 
 . " WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "'");
 ...
 . " WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `pending_reason` != 'authorization' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");
	...
	public function totalRefundedOrder($paypal_iframe_order_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `" . DB_PREFIX . "paypal_iframe_order_transaction` " 
 . " WHERE `paypal_iframe_order_id` = '" . (int)$paypal_iframe_order_id . "' AND `payment_status` = 'Refunded'");
	...
	public function totalRefundedTransaction($transaction_id) {
		$qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `" . DB_PREFIX . "paypal_iframe_order_transaction` " 
 . " WHERE `parent_transaction_id` = '" . $this->db->escape($transaction_id) . "' AND `payment_type` = 'refund'");

		return $qry->row['amount'];
	}

	public function getOrder($order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "paypal_iframe_order` " 
 ...
 . " WHERE paypal_iframe_order_transaction_id = " . (int)$transaction['paypal_iframe_order_transaction_id'] . " ");
	...
	public function addTransaction($transaction_data, $request_data = array()) {
		...
		if ($request_data) {
			...
			$this->db->query(" UPDATE " . DB_PREFIX . "paypal_iframe_order_transaction SET call_data = '" . $this->db->escape($serialized_data) . "' " 
 . " WHERE paypal_iframe_order_transaction_id = " . (int)$paypal_iframe_order_transaction_id . " LIMIT 1 ");
		}
	...
	public function getOrderId($transaction_id) {
		$qry = $this->db->query("SELECT `o`.`order_id` FROM `" . DB_PREFIX . "paypal_iframe_order_transaction` `ot` " 
 . " LEFT JOIN `" . DB_PREFIX . "paypal_iframe_order` `o` ON `o`.`paypal_iframe_order_id` = `ot`.`paypal_iframe_order_id` " 
 . " WHERE `ot`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	...
	public function updateAuthorizationId($paypal_iframe_order_id, $authorization_id) {
		$this->db->query(" UPDATE `" . DB_PREFIX . "paypal_iframe_order` SET `authorization_id` = '" . $this->db->escape($authorization_id) . "' " 
 . " WHERE `paypal_iframe_order_id` = '" . $this->db->escape($paypal_iframe_order_id) . "' ");
	...
	public function updateRefundTransaction($transaction_id, $transaction_type) {
		$this->db->query("UPDATE `" . DB_PREFIX . "paypal_iframe_order_transaction` SET `payment_status` = '" . $this->db->escape($transaction_type) . "' " 
 . " WHERE `transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
	}

	public function getFailedTransaction($paypl_iframe_order_transaction_id) {
		$result = $this->db->query(" SELECT * FROM " . DB_PREFIX . "paypal_iframe_order_transaction " 
 . " WHERE paypal_iframe_order_transaction_id = " . (int)$paypl_iframe_order_transaction_id . " ")->row;
	...
	public function getLocalTransaction($transaction_id) {
		$result = $this->db->query(" SELECT * FROM " . DB_PREFIX . "paypal_iframe_order_transaction " 
 . " WHERE transaction_id = '" . $this->db->escape($transaction_id) . "' ")->row;
