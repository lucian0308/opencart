...
class ModelPaymentRealex extends Model {
	public function install() {
		$this->db->query(" CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "realex_order` ( `realex_order_id` INT(11) NOT NULL AUTO_INCREMENT, `order_id` INT(11) NOT NULL, `order_ref` CHAR(50) NOT NULL, `order_ref_previous` CHAR(50) NOT NULL, `pasref` VARCHAR(50) NOT NULL, `pasref_previous` VARCHAR(50) NOT NULL, `date_added` DATETIME NOT NULL, `date_modified` DATETIME NOT NULL, `capture_status` INT(1) DEFAULT NULL, `void_status` INT(1) DEFAULT NULL, `settle_type` INT(1) DEFAULT NULL, `rebate_status` INT(1) DEFAULT NULL, `currency_code` CHAR(3) NOT NULL, `authcode` VARCHAR(30) NOT NULL, `account` VARCHAR(30) NOT NULL, `total` DECIMAL( 10, 2 ) NOT NULL, PRIMARY KEY (`realex_order_id`) ) ENGINE=MyISAM DEFAULT COLLATE=utf8_general_ci;");
	...
	public function updateVoidStatus($realex_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "realex_order` SET `void_status` = '" . (int)$status . "' " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "'");
	...
	public function updateCaptureStatus($realex_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "realex_order` SET `capture_status` = '" . (int)$status . "' " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "'");
	}

	public function updateForRebate($realex_order_id, $pas_ref, $order_ref) {
		$this->db->query("UPDATE `" . DB_PREFIX . "realex_order` SET `order_ref_previous` = '_multisettle_" . $this->db->escape($order_ref) . "', `pasref_previous` = '" . $this->db->escape($pas_ref) . "' " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "' LIMIT 1");
	}
	...
	public function updateRebateStatus($realex_order_id, $status) {
		$this->db->query("UPDATE `" . DB_PREFIX . "realex_order` SET `rebate_status` = '" . (int)$status . "' " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "'");
	}
	...
	public function getOrder($order_id) {
		...
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "realex_order` " 
 . " WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
	...
	private function getTransactions($realex_order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "realex_order_transaction` " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "'");
	...
	public function getTotalCaptured($realex_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "realex_order_transaction` " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "' AND (`type` = 'payment' OR `type` = 'rebate')");
	...
	public function getTotalRebated($realex_order_id) {
		$query = $this->db->query("SELECT SUM(`amount`) AS `total` FROM `" . DB_PREFIX . "realex_order_transaction` " 
 . " WHERE `realex_order_id` = '" . (int)$realex_order_id . "' AND 'rebate'");
