...
class ModelLocalisationCurrency extends Model {
 ...
 . " WHERE code != '" . $this->db->escape($this->config->get('config_currency')) . "'");
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "currency " 
 . " WHERE code != '" . $this->db->escape($this->config->get('config_currency')) . "' AND date_modified < '" . $this->db->escape(date('Y-m-d H:i:s', strtotime('-1 day'))) . "'");
			}
			...
			foreach ($lines as $line) {
				...
				if ((float)$value) {
					$this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '" . (float)$value . "', date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' " 
 . " WHERE code = '" . $this->db->escape($currency) . "'");
				}
			}

			$this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '1.00000', date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' " 
 . " WHERE code = '" . $this->db->escape($this->config->get('config_currency')) . "'");
