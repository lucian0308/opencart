...
class ModelLocalisationReturnAction extends Model {
 ...
 . " WHERE return_action_id = '" . (int)$return_action_id . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
 ...
 . " ORDER BY name";
			...
			if (!$return_action_data) {
				$query = $this->db->query("SELECT return_action_id, name FROM " . DB_PREFIX . "return_action " 
 . " WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' " 
 . " ORDER BY name");
