...
class ModelLocalisationZone extends Model {
	...
	public function editZone($zone_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "zone SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "', country_id = '" . (int)$data['country_id'] . "' " 
 ...
 . " WHERE zone_id = '" . (int)$zone_id . "'");
	...
	public function getZone($zone_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "zone " 
 . " WHERE zone_id = '" . (int)$zone_id . "'");
	...
	public function getZones($data = array()) {
		$sql = "SELECT *, z.name, c.name AS country FROM " . DB_PREFIX . "zone z " 
 ...
 . " WHERE country_id = '" . (int)$country_id . "' AND status = '1' " 
 . " ORDER BY name");
