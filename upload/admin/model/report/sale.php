...
class ModelReportSale extends Model {
 ...
 . " GROUP BY o.payment_country_id");
	...
	public function getTotalOrdersByDay() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, HOUR(date_added) AS hour FROM `" . DB_PREFIX . "order` " 
 . " WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) = DATE(NOW()) " 
 ...
 . " ORDER BY date_added ASC");
	...
	public function getTotalOrdersByWeek() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` " 
 . " WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= DATE('" . $this->db->escape(date('Y-m-d', $date_start)) . "') " 
 . " GROUP BY DAYNAME(date_added)");
	...
	public function getTotalOrdersByMonth() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` " 
 . " WHERE order_status_id IN(" . implode(",", $implode) . ") AND DATE(date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' " 
 . " GROUP BY DATE(date_added)");
	...
	public function getTotalOrdersByYear() {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, date_added FROM `" . DB_PREFIX . "order` " 
 . " WHERE order_status_id IN(" . implode(",", $implode) . ") AND YEAR(date_added) = YEAR(NOW()) " 
 . " GROUP BY MONTH(date_added)");
	...
	public function getOrders($data = array()) {
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, COUNT(*) AS `orders`, SUM((SELECT SUM(op.quantity) FROM `" . DB_PREFIX . "order_product` op " 
 . " WHERE op.order_id = o.order_id " 
 . " GROUP BY op.order_id)) AS products, SUM((SELECT SUM(ot.value) FROM `" . DB_PREFIX . "order_total` ot " 
 . " WHERE ot.order_id = o.order_id AND ot.code = 'tax' " 
 . " GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total` FROM `" . DB_PREFIX . "order` o";
 ...
 . " WHERE order_status_id > '0'";
	...
	public function getTaxes($data = array()) {
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `" . DB_PREFIX . "order` o " 
