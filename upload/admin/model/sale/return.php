...
class ModelSaleReturn extends Model {
 ...
 . " WHERE return_id = '" . (int)$return_id . "'");
	...
	public function deleteReturn($return_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "return` " 
 . " WHERE return_id = '" . (int)$return_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "return_history " 
 . " WHERE return_id = '" . (int)$return_id . "'");
	}

	public function getReturn($return_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c " 
 . " WHERE c.customer_id = r.customer_id) AS customer FROM `" . DB_PREFIX . "return` r " 
 . " WHERE r.return_id = '" . (int)$return_id . "'");
	...
	public function getReturns($data = array()) {
		$sql = "SELECT *, CONCAT(r.firstname, ' ', r.lastname) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs " 
 . " WHERE rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status FROM `" . DB_PREFIX . "return` r";
 ...
 . " WHERE return_action_id = '" . (int)$return_action_id . "'");
	...
	public function addReturnHistory($return_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "return` SET return_status_id = '" . (int)$data['return_status_id'] . "', date_modified = NOW() " 
 . " WHERE return_id = '" . (int)$return_id . "'");
		...
		if ($data['notify']) {
			$return_query = $this->db->query("SELECT *, rs.name AS status FROM `" . DB_PREFIX . "return` r " 
 . " LEFT JOIN " . DB_PREFIX . "return_status rs ON (r.return_status_id = rs.return_status_id) " 
 . " WHERE r.return_id = '" . (int)$return_id . "' AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getReturnHistories($return_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT rh.date_added, rs.name AS status, rh.comment, rh.notify FROM " . DB_PREFIX . "return_history rh " 
 . " LEFT JOIN " . DB_PREFIX . "return_status rs ON rh.return_status_id = rs.return_status_id " 
 . " WHERE rh.return_id = '" . (int)$return_id . "' AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "' " 
 . " ORDER BY rh.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function getTotalReturnHistories($return_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "return_history " 
 . " WHERE return_id = '" . (int)$return_id . "'");
