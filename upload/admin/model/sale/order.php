...
class ModelSaleOrder extends Model {
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT CONCAT(c.firstname, ' ', c.lastname) FROM " . DB_PREFIX . "customer c " 
 . " WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o " 
 ...
 . " WHERE zone_id = '" . (int)$order_query->row['shipping_zone_id'] . "'");
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os " 
 . " WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
 ...
 . " WHERE order_id = '" . (int)$order_id . "' AND order_option_id = '" . (int)$order_option_id . "'");
 ...
 . " WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product_id . "'");
	...
	public function getOrderVouchers($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_voucher " 
 . " WHERE order_id = '" . (int)$order_id . "'");
 ...
 . " WHERE invoice_prefix = '" . $this->db->escape($order_info['invoice_prefix']) . "'");
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET invoice_no = '" . (int)$invoice_no . "', invoice_prefix = '" . $this->db->escape($order_info['invoice_prefix']) . "' " 
 . " WHERE order_id = '" . (int)$order_id . "'");
 ...
 . " WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0' LIMIT " . (int)$start . "," . (int)$end);
	...
	public function getTotalEmailsByProductsOrdered($products) {
		...
		$query = $this->db->query("SELECT DISTINCT email FROM `" . DB_PREFIX . "order` o " 
 . " LEFT JOIN " . DB_PREFIX . "order_product op ON (o.order_id = op.order_id) " 
 . " WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0'");
