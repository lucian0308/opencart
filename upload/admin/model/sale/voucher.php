...
class ModelSaleVoucher extends Model {
 ...
 . " WHERE voucher_id = '" . (int)$voucher_id . "'");
	...
	public function deleteVoucher($voucher_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "voucher " 
 . " WHERE voucher_id = '" . (int)$voucher_id . "'");
 ...
 . " WHERE voucher_id = '" . (int)$voucher_id . "'");
	...
	public function getVoucher($voucher_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "voucher " 
 . " WHERE voucher_id = '" . (int)$voucher_id . "'");
 ...
 . " WHERE code = '" . $this->db->escape($code) . "'");
	...
	public function getVouchers($data = array()) {
		$sql = "SELECT v.voucher_id, v.code, v.from_name, v.from_email, v.to_name, v.to_email, (SELECT vtd.name FROM " . DB_PREFIX . "voucher_theme_description vtd " 
 . " WHERE vtd.voucher_theme_id = v.voucher_theme_id AND vtd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS theme, v.amount, v.status, v.date_added FROM " . DB_PREFIX . "voucher v";
 ...
 . " WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
	...
	public function getVoucherHistories($voucher_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT vh.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, vh.amount, vh.date_added FROM " . DB_PREFIX . "voucher_history vh " 
 . " LEFT JOIN `" . DB_PREFIX . "order` o ON (vh.order_id = o.order_id) " 
 . " WHERE vh.voucher_id = '" . (int)$voucher_id . "' " 
 . " ORDER BY vh.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
